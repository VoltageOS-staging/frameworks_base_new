...
public class AxPcModeService extends SystemService implements IAxPcModeService {
    ...
    public AxPcModeService(Context context) {
        super(context);
        mContext = context;
    }

    public static IAxPcModeService getService() {
        final IAxPcModeService service = LocalServices.getService(IAxPcModeService.class);
        return service != null ? service : IAxPcModeService.NO_OP;
    }

    @Override
    public void onStart() {
        mHandler = new Handler(Looper.getMainLooper());
        LocalServices.addService(IAxPcModeService.class, this);
    }
    ...
    private void updateDisplayOffState() {
        ...
        final boolean mirrored = dmi != null && dmi.isDefaultDisplayMirrored();
    ...
    private void setInternalDisplayPowerMode(boolean on) {
        ...
        BackgroundThread.getHandler().post(() -> {
            if (mInternalDisplayToken == null) {
                final long[] physicalDisplayIds = DisplayControl.getPhysicalDisplayIds();
                if (physicalDisplayIds != null && physicalDisplayIds.length > 0) {
                    mInternalDisplayToken =
                            DisplayControl.getPhysicalDisplayToken(physicalDisplayIds[0]);
                }
            }

            if (mInternalDisplayToken != null) {
                final int mode = on ? SurfaceControl.POWER_MODE_NORMAL
                        : SurfaceControl.POWER_MODE_OFF;
    ...
    private void updatePcModeState(boolean start) {
        try {
            if (start) {
                ...
                launcherIntent.setComponent(new ComponentName(AX_PC_MODE_PKG, AX_PC_MODE_ACTIVITY));
    ...
    private void onTargetDisplayChanged() {
        final int displayId = getStoredTargetDisplayId();
    ...
    private int getTargetDisplayId() {
        final int stored = getStoredTargetDisplayId();
        if (stored != Display.INVALID_DISPLAY) {
            return stored;
        }
    ...
    private void forceThreeButtonNavigation() {
        final String currentKey = getCurrentSystemNavigationMode();
        final String savedKey = Settings.Secure.getStringForUser(
                mContentResolver,
    ...
    private void forceDesktopDensity(int displayId, boolean saveCurrent) {
        ...
        BackgroundThread.getHandler().post(() -> {
            try {
                ...
                final boolean isSecondary = displayId != Display.DEFAULT_DISPLAY;
                final String densityKey = isSecondary
                        ? SETTING_SECONDARY_DENSITY_KEY
                        : SETTING_DENSITY_OVERRIDE_KEY;
                final int defaultDpi = isSecondary ? DEFAULT_SECONDARY_DPI : DESKTOP_MODE_DPI;
                final int targetDpi = Settings.Secure.getIntForUser(
                        mContentResolver,
                        densityKey,
                        defaultDpi,
                        UserHandle.USER_CURRENT);
                ...
                if (saveCurrent) {
                    ...
                    if (currentDensity == targetDpi) {
                        currentDensity = mIWindowManager.getInitialDisplayDensity(displayId);
                    }
                    Settings.Secure.putIntForUser(
                            mContentResolver,
                            ...
                            currentDensity,
                            UserHandle.USER_CURRENT);
                    Slog.d(TAG, "Saved density: " + currentDensity + " (display " + displayId + ")");
                }
                ...
                mIWindowManager.setForcedDisplayDensityForUser(
                        ...
                        targetDpi,
                        UserHandle.USER_CURRENT);
                Slog.d(TAG, "Forced desktop density: " + targetDpi + " on display " + displayId);
            } catch (RemoteException e) {
    ...
    private void restoreDisplayDensity(int displayId) {
        ...
        final String key = savedDensityKey(displayId);
        BackgroundThread.getHandler().post(() -> {
            try {
                final int savedDensity = Settings.Secure.getIntForUser(
                        mContentResolver, key, -1, UserHandle.USER_CURRENT);

                if (savedDensity != -1) {
                    final int initialDensity = mIWindowManager.getInitialDisplayDensity(displayId);
    ...
    public boolean isPcModeEnabled() {
        return mSystemReady && mPcModeEnabled;
    }
    ...
    public boolean isSecondaryDisplayOnly() {
        ...
        final int id = getStoredTargetDisplayId();
    ...
    private void updateResolutionOverride(int displayId) {
        if (mIWindowManager == null || !mPcModeEnabled) {
            return;
        }

        BackgroundThread.getHandler().post(() -> {
            final String resolutionString = Settings.Secure.getStringForUser(
                    mContentResolver,
            ...
            try {
                ...
                final String[] parts = resolutionString.split("x|X");
                if (parts.length != 2) {
    ...
    public void onDisplayAdded(int displayId) {
        ...
        mHandler.post(() -> setTargetDisplayId(displayId));
    }
    ...
    public void onDisplayRemoved(int displayId) {
        ...
        final int storedId = getStoredTargetDisplayId();
        if (storedId == displayId) {
            Slog.i(TAG, "Target display removed (" + displayId + "), clearing and disabling app");
            mHandler.post(() -> setTargetDisplayId(Display.INVALID_DISPLAY));
        }
    }
