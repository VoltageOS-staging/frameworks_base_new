...
public class HazeRenderer {
    ...
    private void loadTextures(Bitmap bitmap, int w, int h) {
        ...
        mTempTextureId = createEmptyTexture(w, h);
        mBlurTextureId = gpuBlur(mSharpTextureId, w, h, 200f);

        Bitmap blurredBitmap = downloadTexture(mBlurTextureId, w, h);
    ...
    public void drawFrame(float blurStrength, float dimLevel, boolean enableBlobs) {
        ...
        GLES30.glUniform1i(GLES30.glGetUniformLocation(mProgramId, "uEnableBlobs"), enableBlobs ? 1 : 0);
    ...
    public void destroy() {
        GLES30.glDeleteTextures(3, new int[]{mSharpTextureId, mBlurTextureId, mTempTextureId}, 0);
    ...
    private int gpuBlur(int input, int w, int h, float radius) {
        int output = createEmptyTexture(w, h);
        ...
        // Pass 1: Horizontal
        GLES30.glFramebufferTexture2D(GLES30.GL_FRAMEBUFFER, GLES30.GL_COLOR_ATTACHMENT0, GLES30.GL_TEXTURE_2D, mTempTextureId, 0);
        GLES30.glActiveTexture(GLES30.GL_TEXTURE0);
        ...
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, mTempTextureId);
    ...
    private Bitmap downloadTexture(int t, int w, int h) {
        ...
        buf.rewind(); b.copyPixelsFromBuffer(buf);
        return b;
    }
