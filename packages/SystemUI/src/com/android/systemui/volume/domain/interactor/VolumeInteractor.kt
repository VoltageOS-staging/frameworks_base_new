...
class VolumeInteractor @Inject constructor(@Application private val context: Context) {
    private companion object {
        const val TAG = "VolumeInteractor"
    }

    private val audioManager = context.getSystemService(Context.AUDIO_SERVICE) as AudioManager

    private val setRingerModeInternalMethod by lazy {
       try {
            AudioManager::class.java.getMethod("setRingerModeInternal", Int::class.javaPrimitiveType)
        } catch (e: NoSuchMethodException) {
           Log.w(TAG, "setRingerModeInternal not found", e)
            null
        }
    }

    private val getRingerModeInternalMethod by lazy {
        try {
            AudioManager::class.java.getMethod("getRingerModeInternal")
        } catch (e: NoSuchMethodException) {
            Log.w(TAG, "getRingerModeInternal not found", e)
            null
        }
    }
    ...
    fun getRingerMode(): Int {
        return try {
            getRingerModeInternalMethod?.invoke(audioManager) as? Int ?: audioManager.ringerMode
        } catch (e: Exception) {
            Log.e(TAG, "Error calling getRingerModeInternal", e)
    ...
    fun setRingerMode(mode: Int) {
        try {
            setRingerModeInternalMethod?.invoke(audioManager, mode)
        } catch (e: Exception) {
            Log.e(TAG, "Error calling setRingerModeInternal, falling back to public API", e)
