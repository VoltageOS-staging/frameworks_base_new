...
constructor(
    ...
    var brightnessMirrorVisible: Boolean = false
        set(value) {
            ...
            field = value
            brightnessMirrorSpring.animateTo(if (value) blurRadiusOfRatio(1f) else 0)
        }
    ...
    private fun blurRadiusOfRatio(ratio: Float): Int {
        return (ratio * userDefinedMaxBlurRadius).toInt()
    }

    private fun ratioOfBlurRadius(radius: Float): Float {
    ...
    private fun computeBlurAndZoomOut(): Pair<Int, Float> {
        ...
        combinedBlur = max(combinedBlur, blurRadiusOfRatio(qsExpandedRatio).toFloat())
        combinedBlur = max(combinedBlur, blurRadiusOfRatio(transitionToFullShadeProgress).toFloat())
    ...
    private fun blurRadiusToZoomOut(blurRadius: Float): Float {
        var zoomOut = MathUtils.saturate(ratioOfBlurRadius(blurRadius))
                    ...
                    ValueAnimator.ofFloat(1f, 0f).apply {
                        ...
                        addUpdateListener { animation: ValueAnimator ->
                             wakeAndUnlockBlurData =
                                WakeAndUnlockBlurData(blurRadiusOfRatio(animation.animatedValue as Float).toFloat())
                        }
                        addListener(
    ...
    private fun getNewWakeBlurRadius(ratio: Float): Float {
        return if (!wallpaperSupportsAmbientMode) {
            ...
            blurRadiusOfRatio(ratio).toFloat()
        }
    }
    ...
    private fun animateBlur(blur: Boolean, velocity: Float) {
        ...
        shadeAnimation.animateTo(blurRadiusOfRatio(targetBlurNormalized))
    }
    ...
    inner class DepthAnimation() {
        ...
        val ratio
            get() = this@NotificationShadeDepthController.ratioOfBlurRadius(radius)
