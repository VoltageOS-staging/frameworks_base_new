...
public class BatteryInfoNotificationController implements CoreStartable {
    ...
    private void disable() {
        ...
        // Serialize the cancel on the bg handler so it always runs after any
        // already-queued refresh; with the mEnabled guard in
        // doRefreshNotification this prevents a stale re-post after disable.
        mBgHandler.post(() -> {
    ...
    /**
     * Idle (screen-off) drain as a since-charge average. Screen-off counters
     * advance only while the screen is off, so a short rolling window is not
     * meaningful here; the lifetime average is the useful figure.
     */
    ...
    /**
     * Active (screen-on) drain from the live current sensor, averaged over a
     * rolling window. The coulomb-counter mAh field is integer-quantized and
     * barely moves over ~60s at low load, so it is unreliable for a live rate;
     * current_now (already polled every POLL_INTERVAL_MS ms) is smooth.
     */
    private void updateActiveRateFromCurrent() {
        if (mPlugged != 0) {
            // On power: charge current isn't drain. Freeze the last good value
            // (like FKM) instead of blanking it to an en-dash.
    ...
    private double batteryCapacityMah(BatterySummaryStats s) {
        // Derive capacity from observed discharge: total mAh / total fraction.
        // Self-consistent and avoids learned/estimated capacity, which some
        // devices misreport (e.g. ~240 mAh instead of the real pack size).
        ...
        // Fall back to the fuel-gauge charge counter (uAh now) scaled by level.
    ...
    /** Adds wall-time since the last stats snapshot to the matching screen bucket. */
    ...
    private void refreshNotification() {
        // Always build/dedupe/notify on the bg handler so mLastNotifText is
        // touched from a single thread (callers run on both main and bg).
    ...
    private String buildNowLine() {
        // FKM-style: current and temperature only (no wattage).
    ...
    private String statusLabel() {
        // Use plugged state, not EXTRA_STATUS: some ROMs leave STATUS stuck on
        // CHARGING after unplug, but EXTRA_PLUGGED flips to 0 immediately.
        if (mPlugged == 0) {
            return "Discharging";
        }
